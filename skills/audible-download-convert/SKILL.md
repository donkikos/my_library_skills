...
| `AUDIBLE_CONFIG_DIR` | Audible CLI default when unset | Optional location for persistent Audible profiles and authentication configuration. |
| `AUDIBLE_AUTHCODE_FILE` | Converter fallback: `.authcode`, then `$HOME/.authcode` | Optional explicit activation-bytes file; fail if the configured file is unreadable. |
